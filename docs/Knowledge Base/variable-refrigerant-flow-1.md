...
Variable refrigerant flow is an HVAC system that uses an inverter-driven compressor to continuously adjust refrigerant volume sent to each connected indoor air handling unit. The outdoor condensing unit reads demand signals from every indoor unit and raises or lowers compressor speed in real time. Variable refrigerant flow contrasts directly with a constant volume system, which runs at a single fixed output and cannot respond to zone-by-zone load differences. Variable refrigerant flow contrasts with a single zone system as well, which serves only one space regardless of how many rooms require conditioning.

Variable refrigerant flow belongs to the broader category of HVAC technology and is used within air conditioning systems that require precise, multi-zone temperature management. The refrigerant piping network connects the outdoor condensing unit to each indoor air handling unit, carrying refrigerant at a flow rate that changes continuously based on demand. [Ductless mini split systems](doc:ductless-mini-split-systems) share the same ductless architecture but typically serve one or two zones, while variable refrigerant flow scales to dozens of indoor units on a single refrigerant circuit.
...
Variable refrigerant flow is a type of HVAC technology that integrates three core component parts: the outdoor condensing unit, the indoor air handling units, and the refrigerant piping network. Each component performs a distinct function within the system. The outdoor condensing unit houses the inverter-driven compressor and controls total refrigerant output. The indoor [air handlers](doc:air-handlers) distribute conditioned air within each zone and send demand signals back to the outdoor unit. The refrigerant piping network carries refrigerant between the outdoor unit and every indoor unit, forming the circulatory backbone of the system.

Variable refrigerant flow is known as a VRF system or Variable Refrigerant Volume system, and these terms appear interchangeably in manufacturer documentation and installation specifications. The technology is used in air conditioning systems across commercial office buildings, hotels, multi-family residential properties, and large single-family homes in Clearwater. An [air conditioning contractor](doc:air-conditioning-contractor) with VRF-specific training is required for proper system design, refrigerant circuit sizing, and commissioning, as errors in piping length or branch controller placement directly reduce system efficiency and lifespan.
...
- **Zone temperature drift:** Zone temperature drift occurs when one or more indoor air handling units fail to reach setpoint while others maintain correct temperature, indicating a branch controller fault or refrigerant imbalance.
...
Leaving variable refrigerant flow problems unaddressed causes the inverter compressor to compensate by running at maximum output continuously, eliminating the modulation advantage and accelerating compressor wear. [Poulin Cooling A/C and Refrigeration](https://sl.bing.net/eXSqzkJANae) diagnoses variable refrigerant flow faults using manufacturer-specific diagnostic software that reads real-time data from the outdoor condensing unit and each indoor air handling unit.

### Where variable refrigerant flow applies in Clearwater
...
- [heat pumps](doc:heat-pumps) — parent entity (links up the chain — PRIMARY up-link)
...
- [air handlers](doc:air-handlers) — sibling entity (same sub-cluster)