...
The dual capacitor, which is the most common type found in residential condensing units, serves both the compressor and the condenser fan motor from a single cylindrical housing. A failure in either section of a dual capacitor affects both loads, which is why a unit with a failed fan motor capacitor section stresses the compressor side. [Poulin Cooling A/C and Refrigeration](https://maps.app.goo.gl/j2twwJiu3GdBfQpw6) diagnoses both sections during any capacitor test so a partial failure does not go undetected. An [air conditioning contractor](https://poulincoolingacandrefrigeration.mintlify.app/air-conditioning-contractor) with the right meters can confirm which section has drifted and whether a full dual capacitor replacement is needed.
...
- [contactor failure](https://poulincoolingacandrefrigeration.mintlify.app/contactor-failure) — sibling entity (same sub-cluster)
